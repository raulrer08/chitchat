<template>
  <div
    ...
    :class="isUser ? 'bg-yellow-500' : 'bg-blue-700'"
  >
    <div>
      <img
    ...
    <div class="flex flex-grow">
      <div
        class="flex flex-grow items-center justify-between mx-4 font-bold text-xl text-blue-500"
      >
        <p>{{ message.text }}</p>
  ...
  <transition @leave="(el, done) => motions.notification.leave(done)">
    <div
      ...
      class="bg-yellow-500 fixed py-4 px-8 rounded-full bottom-16 right-4 z-10"
      v-if="copied"
    >
      <p class="text-blue-900">Message Copied!</p>
    </div>
  </transition>
</template>
